...
def calculate_bunch_source(q_bunch, n_r, n_xi, b_t_bunch):
    ...
    for i in range(n_xi):
        ...
        for j in range(n_r + 2):
            ...
            b_t_bunch[2 + i, 2 + j] = (cumsum - 0.5 * q_ij) * 1. / (0.5 + j)
        ...
        # axis.
        b_t_bunch[2 + i, 2] -= 0.25 * q_bunch[2 + i, 2] * 2.
...
def deposit_bunch_charge(
    ...
    return deposit_3d_distribution(
        z, x, y, w, xi_grid[0], r_grid[0], n_xi,
        n_r, dxi, dr, q_bunch, p_shape=p_shape,
        use_ruyten=True, r_min_deposit=r_min_deposit)
