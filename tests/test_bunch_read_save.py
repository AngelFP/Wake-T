...
def test_bunch_read_save():
    ...
    for data_format, file_format in zip(data_formats, file_formats):
        ...
        save_bunch_to_file(
            bunch,
            data_format,
            os.path.join(output_folder, "bunch_{}.{}".format(data_format, file_format)),
        )
        # Read saved bunch.
        file_path = os.path.join(
            output_folder, "bunch_{}.{}".format(data_format, file_format)
        )
...
def test_openpmd_reading():
    ...
    diags.write_diagnostics(time=0.0, dt=0.0)
    ...
    diags.write_diagnostics(time=0.0, dt=0.0, species_list=[bunch_1])
    ...
    diags.write_diagnostics(time=0.0, dt=0.0, species_list=[bunch_1, bunch_2])
    ...
    bunch_saved = get_from_file(file_path, "openpmd", species_name="elec_bunch")
    bunch_saved = get_from_file(file_path, "openpmd", species_name="elec_bunch_2")
